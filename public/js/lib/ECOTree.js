...
ECONode = function (id, pid, dsc, type, w, h, c, bc, target, meta) {
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
...
ECOTree = function (obj, elm) {
	...
	this.root = new ECONode(-1, null, null, 'host', 2, 2);
...
//CANVAS functions...
ECOTree._roundedRound = function (ctx,x,y,radius,rc) {
	ctx.beginPath();
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					ECOTree._roundedRound(this.ctx,node.XPosition-10,node.YPosition+10,5,roundColor);
					this.ctx.restore();
					...
					s.push('>');
					s.push('<font face=Verdana size=1><span class="glyphicon ' + (node.type === 'host'?'glyphicon-hdd':'glyphicon-object-align-horizontal') + '">&nbsp;</span>');
					if (node.canCollapse) {
...
ECOTree.prototype.toString = function () {	
	...
	{
		case "CANVAS":
			s.push('<canvas id="ECOTreecanvas" height=1000 width=1210></canvas>');
			break;
...
ECOTree.prototype.add = function (id, pid, dsc, type, w, h, c, bc, target, meta) {
	...
	var node = new ECONode(id, pid, dsc, type, nw, nh, color, border, tg, metadata);	//New node creation...
	node.nodeParent = pnode;  //Set it's parent
	pnode.canCollapse = false; //It's obvious that now the parent can collapse
...
ECOTree.prototype.setNodeColors = function (nodeid, color, border, roundColor, upd) {
	var dbindex = this.mapIDs[nodeid];
	if (color) this.nDatabaseNodes[dbindex].c = color;
	if (border) this.nDatabaseNodes[dbindex].bc = border;
	if (roundColor) this.nDatabaseNodes[dbindex].rc = roundColor;
	if (upd) this.UpdateTree();
}
